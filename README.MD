...
- Testing: unit tests, integration tests and Airflow DAG validation
...
The project is covered by tests at every level, and every merge into `develop`/`main` is gated by CI:
...
- **Airflow DAG validation** — verifies DAG creation, task structure, dependencies, inlets and outlets inside the Airflow environment.
- **Linting & formatting** — [Ruff](https://docs.astral.sh/ruff/) (`ruff check`, `ruff format --check`)
- **Static typing** — [Mypy](https://mypy-lang.org/) over `backend` and `airflow/dags`
Prerequisites: a running Docker daemon (the database integration tests spin up a real PostgreSQL container via `testcontainers`) and the project dependencies installed.
...
CI runs the full suite — Ruff, Mypy, unit/integration and Airflow tests — on every PR and push to `develop`/`main`.
...
The first start can take a minute or two while Airflow initializes. Follow the logs with `docker compose logs -f` if you want to watch the startup or debug an issue.
...
### Rotating Yandex Geocoder API keys

To process points in sequential batches, put one API key per line in
`data/api_keys.txt`. No additional environment variables are required. If the
file does not exist, `YANDEX_GEOCODER_API_KEY` is used as a single-key pool.

The key pool counts every HTTP attempt atomically, switches to the next key
after 1000 uses, permanently disables keys rejected with HTTP 403, and marks
HTTP 429 keys as exhausted until the next UTC day. Runtime state is stored in
`data/api_keys.state.json`; it contains
only key hashes and counters. When a batch finishes, the requests DAG starts
another batch only if points and key capacity remain.
